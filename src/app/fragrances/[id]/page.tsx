...
import { Plus, Trash2, Save, Calculator, AlertTriangle, CheckCircle2, Info, ShieldCheck, Beaker, Zap, Quote, X, Copy, ArrowLeft } from 'lucide-react';
...
export default function EditFragrancePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const calculateCost = () => {
    ...
    selectedItems.forEach((item, index) => {
      ...
      if (variant && variant.price) {
        ...
        const itemWeightInFormula = (Number(item.parts) / totalParts) * 1000;
  ...
  useEffect(() => {
    // Carica materiali e dati fragranza
    Promise.all([
      fetch('/api/materials').then(res => res.json()),
      fetch(`/api/fragrances/${id}`).then(res => res.json())
    ]).then(([materialsData, fragranceData]) => {
      setMaterials(materialsData);
      setName(fragranceData.name);
      setDescription(fragranceData.description || '');
      setSelectedItems(fragranceData.items.map((item: any) => ({
  ...
  const handleDuplicate = async () => {
    ...
    try {
      ...
      if (response.ok) {
        const newFrag = await response.json();
        router.push(`/fragrances/${newFrag.id}`);
      }
    } catch (error) {
  ...
  const handleSmartPaste = () => {
    ...
    lines.forEach(line => {
      ...
      if (match) {
        ...
        const cleanSearch = materialName.toLowerCase().replace(/[®™]/g, '').replace(/\s+/g, ' ').trim();
        ...
        const foundMaterial = materials.find(m => {
          ...
          return mName === cleanSearch || mName.includes(cleanSearch) || cleanSearch.includes(mName) || mSyns.includes(cleanSearch);
        });
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-40 space-y-4">
        ...
        <p className="text-xs font-bold uppercase tracking-widest text-foreground/40 animate-pulse">Caricamento Studio...</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 relative">
      ...
      <div className={`fixed top-0 right-0 h-full w-full md:w-96 bg-white shadow-2xl z-[100] transform transition-transform duration-500 ease-out border-l border-border flex flex-col ${isAiOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          ...
          {aiMessages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[85%] p-4 rounded-2xl text-sm leading-relaxed shadow-sm ${
                msg.role === 'user' ? 'bg-primary text-white rounded-tr-none' : 'bg-white text-slate-700 border border-border rounded-tl-none'
              }`}>
                ...
                {msg.role === 'assistant' && msg.content.includes('[FORMULA_START]') && (
                  <button onClick={() => applyAiFormula(msg.content)} className="mt-4 w-full bg-emerald-500 text-white py-2 rounded-xl font-bold text-[10px] uppercase tracking-widest hover:bg-emerald-600 transition-all flex items-center justify-center gap-2">
                    <Plus className="w-3 h-3" /> Applica Formula Suggerita
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="p-6 bg-white border-t border-border">
           <div className="relative">
              <textarea placeholder="Chiedi un suggerimento creativo..." className="w-full bg-slate-50 border border-border rounded-2xl p-4 pr-12 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 resize-none h-24" value={aiInput} onChange={(e) => setAiInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); handleAiSend(); } }} />
              <button onClick={handleAiSend} disabled={aiLoading || !aiInput.trim()} className="absolute bottom-4 right-4 p-2 bg-primary text-white rounded-xl hover:bg-primary/90 disabled:opacity-50 transition-all">
      ...
      <button onClick={() => setIsAiOpen(true)} className="fixed bottom-8 right-8 w-16 h-16 bg-gradient-to-br from-primary to-accent rounded-full flex items-center justify-center shadow-2xl shadow-primary/40 hover:scale-110 active:scale-95 transition-all z-50 group">
        <Zap className="w-8 h-8 text-white fill-white/10 group-hover:rotate-12 transition-transform" />
      </button>

      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          <button onClick={() => router.push('/fragrances')} className="p-3 bg-white border border-slate-200 rounded-2xl text-slate-400 hover:text-slate-900 hover:border-slate-900 transition-all shadow-sm group">
            <ArrowLeft className="w-5 h-5 group-hover:scale-110 transition-transform" />
          </button>
          <div>
            <h1 className="text-4xl font-bold tracking-tight text-gradient italic">Edit Formula</h1>
            <p className="text-foreground/60 mt-1">Perfeziona la tua creazione e monitora la conformità.</p>
          </div>
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <div className="flex items-center gap-2 px-4 py-2 bg-foreground/5 rounded-xl border border-border">
             <span className="text-[10px] font-bold uppercase tracking-widest text-foreground/40">Cat. IFRA</span>
             <select className="bg-transparent font-bold text-sm focus:outline-none" value={category} onChange={(e) => setCategory(e.target.value)}>
          ...
          <button onClick={handleDuplicate} className="flex items-center gap-2 bg-white border border-border text-slate-600 px-6 py-3 rounded-xl font-bold hover:bg-slate-50 transition-all shadow-sm">
        ...
        <div className="lg:col-span-8 space-y-6">
          <div className="luxury-card p-6 bg-primary/5 border-primary/20 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setIsSmartPasteOpen(!isSmartPasteOpen)} className="text-[10px] font-bold uppercase text-primary hover:underline">{isSmartPasteOpen ? 'Chiudi' : 'Apri Importatore'}</button>
            </div>
            {isSmartPasteOpen && (
              <div className="space-y-4 animate-in fade-in zoom-in-95 duration-300">
                <textarea placeholder={"Esempio:\nAmbrox Super 45%\nBenzaldehyde 8%\nIso E Super 20%"} className="w-full h-48 bg-background border border-primary/20 rounded-2xl p-4 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary/20 custom-scrollbar shadow-inner" value={smartPasteText} onChange={(e) => setSmartPasteText(e.target.value)} />
                <button onClick={handleSmartPaste} className="w-full bg-primary text-white py-4 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-primary/90 transition-all shadow-lg shadow-primary/20">Analizza e Aggiungi</button>
              </div>
            )}
          </div>

          <div className="luxury-card p-8 space-y-4 bg-card/30 backdrop-blur-xl relative overflow-hidden">
            <input type="text" placeholder="Nome della Fragranza" className="w-full text-3xl font-black bg-transparent border-none focus:outline-none placeholder:text-foreground/10" value={name} onChange={(e) => setName(e.target.value)} />
            <div className="flex items-center gap-2 text-foreground/40">
               <Quote className="w-4 h-4 opacity-30" />
               <textarea placeholder="Note creative o descrizione del concept..." className="w-full bg-transparent border-none focus:outline-none text-sm resize-none h-12" value={description} onChange={(e) => setDescription(e.target.value)} />
            </div>
          </div>
                ...
                return (
                  <div key={index} className={`grid grid-cols-12 items-center p-4 group transition-colors ${calcItem?.isLimiting ? 'bg-amber-500/[0.03]' : 'hover:bg-foreground/[0.01]'}`}>
                    <div className="col-span-4 pr-4">
                      <select className="w-full bg-transparent border-none focus:outline-none font-bold text-sm appearance-none" value={item.materialId} onChange={(e) => updateItem(index, 'materialId', e.target.value)}>
                    ...
                    <div className="col-span-2 px-2">
                      <input type="number" className="w-full bg-foreground/5 border border-transparent focus:border-primary/30 focus:bg-background px-3 py-1.5 rounded-lg text-right font-mono text-sm transition-all" value={item.parts} onChange={(e) => updateItem(index, 'parts', e.target.value)} />
                    </div>
                    <div className="col-span-2 text-center">
                       {calcItem ? (
                         <div className="flex flex-col items-center">
                            <span className={`text-[11px] font-bold ${calcItem.isNoRestriction ? 'text-emerald-500 italic' : (calcItem.hasNoData ? 'text-red-500' : 'text-primary')}`}>{calcItem.limitText}</span>
                         </div>
                       ) : <span className="text-[10px] text-foreground/20">---</span>}
                    </div>
                    <div className="col-span-1 text-center font-mono text-[11px] text-foreground/60">{calcItem ? `${calcItem.formulaPercent.toFixed(2)}%` : '0%'}</div>
                    <div className="col-span-2 text-center">
                      {calcItem && calcItem.maxFragranceUsage !== null ? (
                        <div className="flex flex-col items-center">
                           <span className={`text-[11px] font-black ${calcItem.isLimiting ? 'text-amber-500' : 'text-foreground/70'}`}>{calcItem.maxFragranceUsage.toFixed(2)}%</span>
                        </div>
                      ) : <span className="text-[10px] text-foreground/20">---</span>}
                    </div>
                    <div className="col-span-1 text-right">
                      <button onClick={() => removeItem(index)} className="p-2 text-red-500/20 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
                );
              })}
            </div>
            <button onClick={addItem} className="w-full py-6 flex items-center justify-center gap-2 text-sm font-bold text-primary hover:bg-primary/5 transition-colors border-t border-border">
          ...
          <div className="luxury-card overflow-hidden shadow-2xl shadow-primary/10">
            <div className="p-4 bg-gradient-to-br from-primary to-accent text-white">
               <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-bold uppercase tracking-widest opacity-80">IFRA Report</span>
               </div>
            </div>
            <div className="p-8 space-y-8">
              <div className="text-center space-y-2">
                <div className="text-[10px] font-bold uppercase tracking-widest text-foreground/40">Limite Massimo</div>
              ...
              <div className="space-y-4">
                {calculation?.limitingComponent && (
                  <div className="p-5 bg-amber-500/5 border border-amber-500/20 rounded-2xl space-y-3">
                    ...
                    <div className="text-sm font-bold text-primary">{calculation?.limitingComponent?.limitText}</div>
                  </div>
                )}
              </div>
            </div>
